...
def parse_args():
    ...
    ap.add_argument("--obj_out", type=str, default=None, help="Output file for objective values (callback and final).")
...
def _prepare_milp_dump_callback(model, var_r_map, n, directed, base_out_path, obj_out_file=None):
    """
    ...
    model._dump_cb_obj_out = obj_out_file
    ...
    def _cb(m, where):
        if where == GRB.Callback.MIPSOL:
            ...
            # Write objective to file if provided
            if obj is not None and m._dump_cb_obj_out:
                try:
                    with open(m._dump_cb_obj_out, "a") as f:
                        f.write(f"CALLBACK,{obj}\n")
                except Exception:
                    pass
...
def build(n, r, binary, start_mat, args):
    ...
    for i in range(n):
        for j in range(n):
            for k in range(n):
                if i == k or j == k:
                    continue
                var_tri_ineq_wyes[(i, j, k)] = model.addVar(lb=0.0, ub=1.0, name=f"t_{i}_{j}_{k}")

    var_unity_wye = model.addVar(lb=0.0, name="u")
    if args.verbose:
    ...
    #
    # Without symmetry: keep the original O(n^2) family of A^T constraints.
    # With translation symmetry: enforce only for canonical sources 'a' (a in [0..g-1])
    # and fold the k->j (incoming) term across all translated (a',b') equivalents, but only
    # from canonical i. This matches the pattern in your other script where the k->j section
    # iterates over equivalent (a',b') for the outer-loop (a,b).
    if sym is None:
        if args.directed:
        ...
        canon = list(range(sym["g"]))  # canonical vertices are the residue-class reps 0..g-1
        trans = sym["trans"]
        m = sym["m"]

        # Iterate only canonical sources a, and all destinations b != a.
        # (If you want even fewer constraints in the undirected case, we can also
        # restrict b to a canonical subset, but this version matches your example.)
        for a in canon:
            for b in range(n):
                if b == a:
                    continue
                ...
                # i->k  (base (a,b) only; a is canonical here)
                # ----
                pos_lhs_vars += [var_tri_ineq_wyes[(a, j, b)] for j in range(n) if j != a and j != b]

                # k->j  (fold across all translated equivalents (a',b') of (a,b))
                # ----
                for t in range(m):
                    a_out = (a + t * trans) % n
                    b_out = (b + t * trans) % n
                    ...
                    for i in canon:
                        if i == a_out or i == b_out:
                            continue
                        pos_lhs_vars.append(var_tri_ineq_wyes[(i, b_out, a_out)])
...
def solve(model, callback=None, obj_out_file=None):
    if callback is None:
        model.optimize()
    else:
    ...
    try:
        ...
        # Write final objective to file if provided
        if obj_out_file:
            try:
                with open(obj_out_file, "a") as f:
                    status_str = "OPTIMAL" if model.Status == GRB.OPTIMAL else "COMPLETION"
                    f.write(f"{status_str},{objval}\n")
            except Exception:
                pass
    except Exception:
        # Even if no solution, try to write status
        if obj_out_file:
            try:
                with open(obj_out_file, "a") as f:
                    f.write(f"NO_SOLUTION,None\n")
            except Exception:
                pass
    return model
...
def main():
    ...
    args.sym, trans_mod, size_canon = validate_and_prepare_translation_symmetry(n, args.trans, args.size_canon)
    ...
    out_path = args.out
    if out_path is None:
        out_name = f"general_{n}_r{r}_{mode}_{dir_tag}"
        if args.sym:
            out_name = f"{out_name}_{trans_mod}trans_{size_canon}canon"
    ...
    if bool(args.binary_r_map):
        cb = _prepare_milp_dump_callback(model, var_r_map, n, directed=args.directed, base_out_path=out_path, obj_out_file=args.obj_out)
        model = solve(model, callback=cb, obj_out_file=args.obj_out)
    ...
    if args.sym is None:
        ...
        for iter_val in range(n_iters):
            ...
            model = solve(model)  # LP: do not log intermediate objectives
            adj = score_solution(model, var_r_map, n, directed=args.directed)
        ...
        model = solve(model, obj_out_file=args.obj_out)
        adj = score_solution(model, var_r_map, n, directed=args.directed)
        output_adj_mat(adj, n, out_path, directed=args.directed, assert_binary=True)
        return
