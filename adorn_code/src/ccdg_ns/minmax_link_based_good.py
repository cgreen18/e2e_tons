...
class CCDGModel:
    ...
    def _constr_flow_to_cdg_mapping_(self):
        M_uv = self.total_network_demand  # Big-M constraint bound
        for (u, v) in self.uv_turn_set:
            known = self.known_load_turns.get((u, v), 0.0)
            flow_sum = gp.quicksum(self.vars_uv_flow[s][(u, v)] for s in self.source_commodities) + known
            self.model.addConstr(flow_sum <= M_uv * self.vars_turn_adj_mat[(u, v)], name=f"f2c_{u}u_{v}v")

    def _constr_physical_link_capacity_(self):
        M_ij = self.total_network_demand  # Big-M constraint bound
        for (i, j) in self._iter_topo_edges():
            phys_link_flow_expr = gp.LinExpr()
            for l in range(self.n_vcs):
                u = self.topo_ijv_to_cdg_u_map[(i, j, l)]
                for s in self.source_commodities:
                    phys_link_flow_expr += self.vars_sink[s][u]
                    for v in self.u_to_v_turns[u]:
                        phys_link_flow_expr += self.vars_uv_flow[s][(u, v)]
            
            known = self.known_load_links.get((i, j), 0.0)
            total_edge_flow = phys_link_flow_expr + known

            # C8a: Min-Max Load Definition
            self.model.addConstr(total_edge_flow <= self.var_lmax, name=f"lmax_{i}i_{j}j")
            
            # C8b: Topology Big-M Enforcement
            self.model.addConstr(total_edge_flow <= M_ij * self.vars_topo_adj_mat[(i, j)], name=f"phys_cap_{i}i_{j}j")
...
class CCDGOptimizer:
    ...
    def __init__(self, ccdg_model, model_params):
        ...
        if self.model_params.get("mip_gap") is not None:
            self.model.Params.MIPGap = float(self.model_params["mip_gap"])
    ...
    def solve(self):
        ...
        if self.model.Status == GRB.INFEASIBLE:
            ...
            raise RuntimeError(f"Infeasible solution. Status={self.model.Status}")
        ...
        print(f"Solve w/ obj (L_max) {self.objval}")
        return self.model
    ...
    @staticmethod
    def accumulate_turn_loads(paths_by_sd, known_load_turns):
        for paths in paths_by_sd.values():
            for p in paths:
                w_k = float(p["W_k"])
                cdg_fwd = p["cdg_nodes"]
                for a, b in zip(cdg_fwd, cdg_fwd[1:]):
                    known_load_turns[(a, b)] += w_k
...
def write_per_source_solve_debug(out_dir, base_name, idx, source, results, cdg_u_to_topo_ijv_map):
...
def print_results(final_lmax_val, final_topology, final_paths, final_link_load, final_routing_table, final_vc_table, capacity, demand):
...
def _link_bottleneck_throughput_proxy(s, dests, demand, hardened_topo_edges, known_load_links, capacity, known_lmax, n_nodes):
    ...
    for (i, j), extra in delta_load.items():
        ...
        if cap_ij <= 0:
            return float("inf")
        l_hat = max(l_hat, (known_load_links.get((i, j), 0.0) + extra) / cap_ij)
    return l_hat
...
def _turn_bottleneck_throughput_proxy(
    ...
    for (u, v), extra in delta_turn.items():
        l_hat = max(l_hat, known_load_turns.get((u, v), 0.0) + extra)
    return l_hat
...
def _moore_throughput_proxy(s, dests, demand, hardened_topo_edges, known_load_links, capacity, known_lmax, n_nodes):
    ...
    for (i, j) in hardened_topo_edges:
        cap_ij = float(capacity[i][j])
        residual_cap += max(0.0, cap_ij - known_load_links.get((i, j), 0.0))
    if residual_cap <= 0:
        return float("inf")
...
def run_per_source_optimization(problem_args, file_args, solver_params):
    ...
    while remaining_sources:
        ...
        topology, paths, link_load, route_table, vc_table = ccdg_optimizer.harden_results()

        merged_paths.update(paths)
        CCDGOptimizer.merge_routing_tables(merged_route_table, merged_vc_table, route_table, vc_table)

        for k, v in link_load.items():
            known_load_links[k] += v
        CCDGOptimizer.accumulate_turn_loads(paths, known_load_turns)
        ...
        cumulative_max_load = max(known_load_links.values()) if known_load_links else 0.0
        print(
    ...
    final_link_load = dict(known_load_links)
...
def define_all_arguments():
    ap = argparse.ArgumentParser(description="One-shot topology, routing, and deadlock freedom synthesis via CDG. Objective: Min-Max Channel Load.")
    ...
    ap.add_argument("--prioritized-per-source", choices=["hops", "throughput"], default=None, help="With --per-source-solves: after iteration 1, reorder remaining sources by hops (highest avg hop distance) or throughput (lowest proxy; see NEXT_SOURCE_THROUGHPUT_PROXY in source).")
    ...
    ap.add_argument("--safe", action="store_true", help="With --per-source-solves: on first iteration only, also harden spanning-tree reverse turns and root-pivot turns.")
    ...
    ap.add_argument("--mip_gap", type=float, default=None, help="Gurobi MIP relative gap tolerance.")
...
def parse_problem_arguments(args):
    ...
    solver_params = {"silent": args.silent, "threads": args.threads, "time_limit": args.time_limit, "mip_gap": args.mip_gap}
...
def main():
    ...
    if problem_args["per_source_solves"]:
        if problem_args["relax_topo_edges"] or problem_args["relax_turn_edges"]:
            print("UNIMPLEMENTED: per-source solves require binary topology and turn edges.")
            sys.exit(1)
        if not problem_args["integral_flow"]:
            print("UNIMPLEMENTED: per-source solves require integral flow.")
            sys.exit(1)
    ...
    print_results(final_lmax_val, final_topology, final_paths, final_link_load, final_routing_table, final_vc_table, problem_args["cap_mat"], problem_args["dem_mat"])
    ...
    if args.out_json:
        ...
        if problem_args["per_source_solves"]:
            ...
            out_payload["lmax_per_source"] = {str(k): v for k, v in lmax_per_source.items()}
