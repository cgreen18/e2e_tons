...
def solve_mcf_nodearc_ncommodity(G, commodities, cap, directed, nodearc_capmode="paper",
    ...
    for si, s in enumerate(src_list):
        for (u, v) in arcs:
            f[(si, u, v)] = m.addVar(lb=0.0, name=f"f[{s},{u},{v}]")
    m.update()

    # Flow conservation
    for si, s in enumerate(src_list):
        for v in range(n):
            expr_out = gp.quicksum(f[(si, u, w)] for (u, w) in out_arcs[v])
            expr_in = gp.quicksum(f[(si, u, w)] for (u, w) in in_arcs[v])
    ...
    if directed:
        ...
        for (u, v) in G.iterEdges():
            ce = cap[undirected_edge_key(u, v)]
            expr = gp.quicksum(f[(si, u, v)] + f[(si, v, u)] for si in range(len(src_list)))
            if capmode == "paper":
    ...
    for si, s in enumerate(src_list):
        print(f"src {s}")
        for (u, v) in arcs:
            # if cap[undirected_edge_key(u, v)] == 0:
            #     continue
            var = f[(si,u,v)]
            val = var.X
            # if val == 0:
            #     continue
            print(f"\tarc {(u,v)} : {var.X}")
    # for k, v in f.items():
    #     print(f"flow {k} : {v.X}")
...
def main():
    ...
    if args.formulation == "edge":
        ...
        res = solve_mcf_nodearc_ncommodity(
            G, commodities, cap, args.directed, nodearc_capmode=args.nodearc_capmode,
            time_limit=args.time_limit, threads=args.threads, verbose=verbose
        )
