...
// g++ -O3 -std=c++17 -fopenmp src/worst_case_parallel.cpp -o bin/worst_case_parallel
...
// This function relies only on local variables, making it naturally thread-safe.
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        cerr << "Usage: ./worst_case_throughput <adjacency_matrix.txt>" << endl;
        return 1;
    }
    ...
    cout << "Calculating worst-case throughput using " 
         << omp_get_max_threads() << " threads..." << endl;

    // Parallelize across all channels using OpenMP.
    // dynamic schedule is used because the matching time can vary per channel.
    #pragma omp parallel for schedule(dynamic)
    for (size_t i = 0; i < channels.size(); ++i) {
        int u = channels[i].first;
        int v = channels[i].second;
        double b_c = adj_matrix[u][v];

        // Cost matrix is initialized inside the loop so it is private to each thread
