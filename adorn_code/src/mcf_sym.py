...
def solve_mcf_edge_based(G, commodities, cap, weighted, directed, time_limit=None, threads=None, verbose=True):
    ...
    if directed:
        ...
        for (u, v) in G.iterEdges():
            ...
            m.addConstr(
                gp.quicksum(f[(k, u, v)] + f[(k, v, u)] for k in range(len(commodities))) <= 2*ce,
                name=f"cap[{u},{v}]"
            )
...
# ----------------------------
# MCF: node-arc (n-commodity, per-source)
# ----------------------------

def solve_mcf_nodearc_ncommodity(G, commodities, cap, directed, xyzc_dims, mc_dims, sym_type="refl-trans", nodearc_capmode="paper",
                                time_limit=None, threads=None, verbose=True):
    """
    Node-arc formulation with ONE commodity per source (n-commodity style).

    This matches Shahrokhi-Matula (1990) Eq. (2.2): for each source i, commodity i
    simultaneously supplies all demands D(i, j) (j != i). Flow conservation is:

        sum_{k in N(v)} ( f_i(v,k) - f_i(k,v) ) =
            lambda * sum_j D(i,j)   if v == i
            -lambda * D(i,v)        otherwise

    Capacity:
      - directed: sum_i f_i(u,v) <= C(u,v)
      - undirected:
          * capmode='paper'   : 0.5 * sum_i (f_i(u,v) + f_i(v,u)) <= C({u,v})
          * capmode='standard':       sum_i (f_i(u,v) + f_i(v,u)) <= C({u,v})

    Notes:
      - We allow arbitrary (possibly asymmetric) demands via the provided commodities list.
        Any missing pair demand is treated as 0.
      - If you restrict --sources/--sinks, the per-source commodity only includes those
        destination demands present in the commodity list.
    """
    n = G.numberOfNodes()
    ...
    print(f"canon nodes : {canon_nodes}")

    # Build arc list used by variables and flow conservation
    arcs = []
    if directed:
        for (u, v) in G.iterEdges():
            arcs.append((u, v))
    ...
    # Build demand map D(s,t) from commodities; group by source
    dem_by_src = defaultdict(dict)
    srcs = set()
    for (s, t, dem) in commodities:
        if s == t:
            continue
        if dem <= 0:
            continue
        srcs.add(s)
        dem_by_src[s][t] = float(dem)

    src_list = sorted(srcs)
    if not src_list:
        raise ValueError("No sources found in commodities (after filtering).")

    total_out = {s: sum(dem_by_src[s].values()) for s in src_list}

    m = gp.Model("MCF_nodearc_ncommodity")
    if not verbose:
        m.Params.OutputFlag = 0
    if time_limit is not None:
        m.Params.TimeLimit = float(time_limit)
    ...
    # Align numeric settings with edge formulation
    m.Params.Method = 2
    ...
    # Flow variables f[src_idx, u, v]
    f = {}
    for si, s in enumerate(src_list):
        if s not in canon_nodes:
            continue
        for (u, v) in arcs:
            u_prime, u_tform = my_tpuv4_symmetry.get_canonical_equivalent(u)
            v_prime = my_tpuv4_symmetry.apply_transformation(v, u_tform)
            if (s, u_prime, v_prime) not in f:
                f[(s, u_prime, v_prime)] = m.addVar(lb=0.0, name=f"f[{s},{u},{v}]")
    ...
    for si, s in enumerate(src_list):
        ...
        for v in range(n):
            ...
            expr_out = gp.LinExpr()
            for (u,w) in out_arcs[v]:
                u_prime, u_tform = my_tpuv4_symmetry.get_canonical_equivalent(u)
                w_prime = my_tpuv4_symmetry.apply_transformation(w, u_tform)

                expr_out += f[(s, u_prime, w_prime)]
            expr_in = gp.LinExpr()
            for (u,w) in in_arcs[v]:
                u_prime, u_tform = my_tpuv4_symmetry.get_canonical_equivalent(u)
                w_prime = my_tpuv4_symmetry.apply_transformation(w, u_tform)

                expr_in += f[(s, u_prime, w_prime)]

            if v == s:
                rhs = lam * float(total_out[s])
            else:
                rhs = -lam * float(dem_by_src[s].get(v, 0.0))

            m.addConstr(expr_out - expr_in == rhs, name=f"flow[{s},{v}]")
    ...
    if directed:
        ...
        for (u, v) in G.iterEdges():
            ce = cap[(u, v)]
            m.addConstr(gp.quicksum(f[(si, u, v)] for si in range(len(src_list))) <= ce,
                        name=f"cap[{u},{v}]")
    else:
        capmode = (nodearc_capmode or "paper").lower()
        if capmode not in ("paper", "standard"):
            raise ValueError("nodearc_capmode must be 'paper' or 'standard'")

        for (u, v) in G.iterEdges():
            # if u not in canon_nodes:
            #     continue
            ce = cap[undirected_edge_key(u, v)]
            ...
            for si, s in enumerate(src_list):
                ...
                u0_prime, u0_tform = my_tpuv4_symmetry.get_canonical_equivalent(u)
                v0_prime = my_tpuv4_symmetry.apply_transformation(v, u0_tform)


                v1_prime, v1_tform = my_tpuv4_symmetry.get_canonical_equivalent(v)
                u1_prime = my_tpuv4_symmetry.apply_transformation(u, v1_tform)

                expr += f[(s, u0_prime, v0_prime)] + f[(s, v1_prime, u1_prime)]

            if capmode == "paper":
                m.addConstr(0.5 * expr <= ce, name=f"cap[{u},{v}]")
            else:
...
def main():
    ap = argparse.ArgumentParser(description="Maximum Concurrent Flow (MCF) in Gurobi: edge-based, path-based, and node-arc (n-commodity) formulations.")
    ...
    ap.add_argument("--formulation", choices=["edge", "path", "nodearc"], default="edge", help="Which formulation to solve.")
    ap.add_argument("--nodearc-capmode", choices=["paper", "standard"], default="paper",
                   help="For undirected node-arc only: capacity constraint mode. "
                        "'paper' matches Shahrokhi-Matula (2.2) with 0.5*sum<=C; "
                        "'standard' uses sum<=C (matches this script's edge formulation).")
    ...
    ap.add_argument('--xyzc_dims',nargs='+',type=int,required=True,help='Global system x, y, z, and cube dimensions. Type without parenthesis and use spaces, no commas')
    ap.add_argument('--mc_dims',nargs='+',type=int,help='Minimum "cube" of symmetry\'s x, y, and z dimensions. If unspecified then assuming single (zeroth) cube (i.e., x,y,z==cube). Type without parenthesis and use spaces, no commas')
    ap.add_argument('--sym_type',type=str,choices=["trans","refl-trans"],default="trans",help='graph is translationally vertex symmetric')
    ...
    elif args.formulation == "nodearc":
        res = solve_mcf_nodearc_ncommodity(
            G, commodities, cap, args.directed, xyzc_dims, mc_dims, sym_type=sym_type, nodearc_capmode=args.nodearc_capmode,
            time_limit=args.time_limit, threads=args.threads, verbose=verbose
        )
        print(f"[nodearc] status={status_str(res['status'])}  lambda={res['lambda']}  capmode={args.nodearc_capmode}  n_src={len(res.get('sources', []))}")
    else:
